...
<html lang="es">
	...
	$(document).ready(function(){
		$('#mitabla').DataTable({
			"order": [[4, "desc"]], //PARA ORDENAR LAS CONSULTAS POR LA FECHA QUE SE HIZO LA CONSULTA DE FORMA DESCENDENTE
			"language":{
					...
					<tbody>
						...
						<?php foreach ($resultado as $row): ?>
							...
							<tr>
								...
								<?php  
								//LOS TRATAMIENTO HAY QUE PRIMERO SUSTITUIR LOS SALTOS DE LINEAS POR / YA QUE SI NO ENTONCES NO SE PUEDE ENVIAR POR LA URL, LUEGO EN RECIPE.PHP LE QUITO LOS / Y LE AGREGO EL SALTO DE LINEA NUEVAMENTE (AQUI CADA SALTO DE LINEA SE SUSTITUYE POR //)
									$tratamientoNuevo =  preg_replace("~[\r\n]~", "/",$row['TRATAMIENTO']);

								?>

								<td><button onclick="window.open('recipe_medico.php?nombre_mascota=<?php echo $row['NOMBRE_MASCOTA'];?>&medicamentos=<?php echo $row['MEDICAMENTOS'];?>&fecha=<?php echo $row['FECHA_CITA'];?>&tratamiento=<?php echo $tratamientoNuevo;?>');"><i class="icon-heartbeat"></i></button></td>									
							</tr>
						<?php endforeach ?>
