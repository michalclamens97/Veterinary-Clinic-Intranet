			...
			<div class="col-md-6">
				<div class="card">
					<div class="prueba card card-inverse">
								...
								<h3 class="card-title">Agregar Nueva<br>Especie</h3>
									<form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" id="form_registro" method="POST">
											<label>Especies Actuales:</label><br>
											<select class="form-control">
												<option value="especie" selected disabled>Especies</option>
												<?php foreach ($resultado as $dato): ?>
												<option value="<?php echo $dato['ESPECIE']; ?>" disabled><?php echo ucwords($dato['ESPECIE']); ?></option>
												<?php endforeach ?>
											</select>
											<br>
											<label for="especie">Ingrese Nueva Especie:</label>
											<input type="text" name="especie" id="especie" class="required form-control"  title="Campo Obligatorio" placeholder="Especie...">
											<br>
											<button type="submit" class="pruebaBoton btn btn-block btn-lg mb-1">Agregar Especie</button>
										</form>
								</div>
...
<script type="text/javascript" src="js/validaLetrasNumeros.js"></script>
 <script type="text/javascript">
            $(function(){
                ...
                $('#especie').validCampoFranz(' abcdefghijklmnñopqrstuvwxyzáéíóú');       
            });
        </script>  
</html>
