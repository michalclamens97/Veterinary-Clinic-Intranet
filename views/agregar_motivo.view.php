...
<head>
	...
	<script type="text/javascript">
			function JSalert(){
				swal("ERROR!", "Ya ese motivo esta registrado!", "warning");
			}
			...
			<div class="col-md-6">
				<div class="card">
					<div class="prueba card card-inverse">
								...
								<h3 class="card-title">Agregar Motivo<br>Consulta</h3>
									<form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" id="form_registro" method="POST">
											<label>Motivos Actuales:</label><br>
											<select class="form-control">
												<option value="motivo" selected disabled>Motivos...</option>
												<?php foreach ($resultado as $dato): ?>
												<option value="<?php echo $dato['MOTIVO']; ?>" disabled><?php echo ucwords($dato['MOTIVO']); ?></option>
												<?php endforeach ?>
											</select>
											<br>
											<label for="motivo">Ingrese Nuevo Motivo:</label>
											<input type="text" name="motivo" id="motivo" class="required form-control"  title="Campo Obligatorio" placeholder="Motivo...">
											<br>
											<button type="submit" class="pruebaBoton btn btn-block btn-lg mb-1">Agregar Motivo</button>
										</form>
								</div>
				...
				<div id="secundario" class="row mt-3">
...
<script type="text/javascript" src="js/validaLetrasNumeros.js"></script>
 <script type="text/javascript">
            $(function(){
                ...
                $('#motivo').validCampoFranz(' abcdefghijklmnñopqrstuvwxyzáéíóú');       
            });
        </script>  
</html>
