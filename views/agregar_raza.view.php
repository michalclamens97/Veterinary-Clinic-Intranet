...
<head>
	...
	<script type="text/javascript">
			function JSalert(){
				swal("ERROR!", ", Ya esa especie esta registrada!", "warning");
			}

			function JSalertRaza(){
				swal("ERROR!", ", Ya esa raza esta registrada!", "warning");
			}
			...
			<div class="col-md-6">
				<div class="card">
					<div class="prueba card card-inverse">
								...
								<h3 class="card-title">Agregar Nueva<br>Raza</h3>
									<form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" id="form_registro" method="POST">
											<label for="especie">Selecciona una especie:</label><br>
											<select class="required form-control" name="especie" id="especie" title="Seleccione una especie">
												<option value="especie" selected disabled>Especies</option>
												<?php foreach ($resultado as $dato): ?>
												<option value="<?php echo $dato['ESPECIE']; ?>"><?php echo ucwords($dato['ESPECIE']); ?></option>
												<?php endforeach ?>
											</select>
											<br>
											<label for="raza">Razas existentes:</label>
											<select name="raza" id="raza" class="form-control">
								
											</select>
											<br>
											<label for="raza">Ingrese Nueva Raza:</label>
											<input type="text" name="raza" id="raza" class="required form-control"  title="Campo Obligatorio" placeholder="Raza...">
											<br>
											<button type="submit" class="pruebaBoton btn btn-block btn-lg mb-1">Agregar Raza</button>
										</form>
								</div>
...
<script type="text/javascript" src="js/select_dinamico2.js"></script>
<!--VALIDACIONES-->
...
<script type="text/javascript" src="js/validaLetrasNumeros.js"></script>
 <script type="text/javascript">
            $(function(){
                ...
                $('#raza').validCampoFranz(' abcdefghijklmnñopqrstuvwxyzáéíóú');       
            });
        </script>  
</html>
