...
<head>
	...
	<script type="text/javascript">
			function JSalert(){
				swal("EXITO!", "Motivo eliminado!", "success");
			}
			...
			<div class="col-md-6">
				<div class="card">
					<div class="prueba card card-inverse">
								...
								<h3 class="card-title">Eliminar Motivo<br>Consulta</h3>
									<form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" id="form_registro" method="POST">
											<label>Seleccione el motivo a eliminar:</label><br>
											<select  name="motivo" id="motivo" class="required form-control"  title="Seleccione un Motivo" >
												<option value="motivo" selected disabled>Motivos...</option>
												<?php foreach ($resultado as $dato): ?>
												<option value="<?php echo $dato['MOTIVO']; ?>"><?php echo ucwords($dato['MOTIVO']); ?></option>
												<?php endforeach ?>
											</select>
											<br>
											<button type="submit" class="pruebaBoton btn btn-block btn-lg mb-1">Eliminar Motivo</button>
										</form>
								</div>
