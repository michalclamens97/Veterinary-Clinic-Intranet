...
<head>
	...
	<script type="text/javascript">
			function JSalert(){
				swal("EXITO!", "Procedimiento eliminado!", "success");
			}
			...
			<div class="col-md-6">
				<div class="card">
					<div class="prueba card card-inverse">
								...
								<h3 class="card-title">Eliminar<br>Procedimiento</h3>
									<form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" id="form_registro" method="POST">
											<label>Seleccione el procedimiento a eliminar:</label><br>
											<select  name="procedimiento" id="procedimiento" class="required form-control"  title="Seleccione un Procedimiento" >
												<option value="procedimiento" selected disabled>Procedimientos...</option>
												<?php foreach ($resultado as $dato): ?>
												<option value="<?php echo $dato['PROCEDIMIENTO']; ?>"><?php echo ucwords($dato['PROCEDIMIENTO']); ?></option>
												<?php endforeach ?>
											</select>
											<br>
											<button type="submit" class="pruebaBoton btn btn-block btn-lg mb-1">Eliminar Procedimiento</button>
										</form>
								</div>
